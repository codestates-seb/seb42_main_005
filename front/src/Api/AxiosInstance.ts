...
import { TYPE_AllUserInfo } from "./TYPES";
...
//* POST : 찜하기/찜취소
export const likePharmacy = async (storeidx: number, value: any, state: any) => {
...
export const findPW = async (findPassword: string) => {
  return BaseInstance.patch(APIS.PATCH_FINDPW, { email: findPassword }, { headers: { Authorization: token } }).catch(
    (error) => {
...
export const deleteReview = async (storeIdx: number, reviewIdx: number) => {
  return BaseInstance.delete(`${APIS.DELETE_REVIEWS}/${storeIdx}/review/${reviewIdx}`, {
    headers: { Authorization: token },
  }).catch((error) => {
    console.log("리뷰 삭제하던 중 에러 발생");
    console.log(error);
...
export const deleteReply = async (reviewIdx: number, replyIdx: number) => {
  return BaseInstance.delete(`${APIS.DELETE_REPLY}/${reviewIdx}/reply/${replyIdx}`, {
    headers: { Authorization: token },
  }).catch((error) => {
